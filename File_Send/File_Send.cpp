...
// 파일을 압축하는 함수
int compress_file(const std::string &file_path, std::string id) {
    std::string comp_file_name = id + "_request";
    // 입력 파일을 이진 모드로 열기
    std::ifstream ifs(file_path.c_str(), std::ios_base::binary);
    // 출력 파일 (압축 파일)을 이진 모드로 생성
    std::ofstream ofs(comp_file_name.c_str(), std::ios_base::binary);
    int i = 0;
    // 버퍼 선언
...
int create_tar(const std::string &dir_path, std::string id) {
    // tar 명령어를 이용하여 디렉토리를 tar 파일로 만드는 명령어 생성
    std::string command = "tar -cf " + id + "_request.tar " + dir_path + " 2> /dev/null";
    // 시스템 명령어 실행
...
void make_file(int server_socket, std::string id, bool *isLoop) { // 함수 정의. send_file 함수는 서버 소켓과 루프 실행 여부를 인자로 받습니다.
    ...
    std::string request_message = "", dir_path = "", tar_path = id + "_request.tar" , file_path = id + "_request", EOF_MSG = "<EOF>"; // 메시지, 경로 등을 저장할 문자열 변수 선언
    int tar_res, comp_res; // tar 파일 생성 및 파일 압축 결과를 저장할 변수 선언
    ...
    while(*isLoop){ // 루프 실행 조건 하에
        ...
        if (FD_ISSET(server_socket , &readfds)) { // 서버 소켓이 읽기 가능한 상태라면
            ...
            tar_res = create_tar(dir_path.c_str(), id); // tar 파일 생성
            ...
            comp_res = compress_file(tar_path.c_str(), id);
            if (comp_res == -1) { // lz4 파일 생성 오류 검사
                if (n <= 0) { // 쓰기 오류 검사
...
void file_send(int server_socket, std::string file_path){
    ...
    std::streamsize size = file_to_send.tellg();
    file_to_send.seekg(0, std::ios::beg);

    // 전송 될 파일 크기 전송
    std::string size_str = std::to_string(size);
    if (send(server_socket, size_str.c_str(), size_str.size(), 0) == -1) {
		printf("파일 크기 전송 실패\n");
		exit(1);
	}
